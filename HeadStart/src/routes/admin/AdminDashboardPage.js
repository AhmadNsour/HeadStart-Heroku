...
export default class AdminDashboardPage extends Component {
  ...
  addAdmin = event => {
    ...
    axios
      .post("https://test-head-start-1.herokuapp.com/add-admin", {
        name,
        email,
        password,
        mobileNumber,
  ...
  getUsers() {
    axios.get("https://test-head-start-1.herokuapp.com/get-users").then(response => {
      if (this._isMounted) {
  ...
  getPendingAdmins() {
    axios.get("https://test-head-start-1.herokuapp.com/get-pending-admins").then(response => {
      if (this._isMounted) {
  ...
  deletePost = _id => {
    if (window.confirm("Delete Post?....")) {
      ...
      axios
        .post("https://test-head-start-1.herokuapp.com/delete-user-post", { _id, user_id })
  ...
  getPendingHrQuestions() {
    axios.get("https://test-head-start-1.herokuapp.com/get-hr-pendings").then(res => {
      if (this._isMounted) {
  ...
  getPendingTechQuestions() {
    axios.get("https://test-head-start-1.herokuapp.com/get-tech-pendings").then(res => {
      if (this._isMounted) {
        this.setState({ pendingQuestions: res.data });
      }
    });
  ...
  answerPending = post => {
    if (window.confirm("Answer Post?....")) {
      axios.post("https://test-head-start-1.herokuapp.com/answer-pending", post).then(res => {
  ...
  acceptPending = admin => {
    if (window.confirm("Are You Sure?....")) {
      axios
        .post("https://test-head-start-1.herokuapp.com/accept-pending", admin)
  ...
  deletePending = _id => {
    axios.post("https://test-head-start-1.herokuapp.com/delete-pending", { _id }).then(res => {
      if (this.props.loggedInUser.role === "owner") {
        this.getPendingAdmins();
      } else if (this.props.loggedInUser.role === "hrAdmin") {
        this.getPendingHrQuestions();
  ...
  deleteUser = _id => {
    if (window.confirm("Are You Sure?....")) {
      axios
        .post("https://test-head-start-1.herokuapp.com/delete-user", { _id })
